...
bool BlobMaterialShader::updateUniformData(RenderState& state, QSGMaterial* newMaterial, QSGMaterial* oldMaterial) {
    ...
    // 1440 = 160 + 16*80 (rectData[80] 结束) ;再加渐变 colorBottom(16) + gradTop(4) + gradBottom(4) + 8 padding = 1472
    ...
    // 渐变 uniforms,接在 rectData 后面(offset 1440 起)
    const float colorBottom[4] = {
        static_cast<float>(mat->m_colorBottom.redF()),
        static_cast<float>(mat->m_colorBottom.greenF()),
        static_cast<float>(mat->m_colorBottom.blueF()),
        static_cast<float>(mat->m_colorBottom.alphaF()),
    };
    memcpy(buf->data() + 1440, colorBottom, 16);
    memcpy(buf->data() + 1456, &mat->m_gradientTop, 4);
    memcpy(buf->data() + 1460, &mat->m_gradientBottom, 4);
