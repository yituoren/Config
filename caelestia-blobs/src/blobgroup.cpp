...
void BlobGroup::setColorBottom(const QColor& c) {
    if (m_colorBottom == c)
        return;
    m_colorBottom = c;
    emit colorBottomChanged();
    markDirty();
}

void BlobGroup::setGradientTop(qreal v) {
    if (qFuzzyCompare(m_gradientTop, v))
        return;
    m_gradientTop = v;
    emit gradientTopChanged();
    markDirty();
}

void BlobGroup::setGradientBottom(qreal v) {
    if (qFuzzyCompare(m_gradientBottom, v))
        return;
    m_gradientBottom = v;
    emit gradientBottomChanged();
    markDirty();
}
